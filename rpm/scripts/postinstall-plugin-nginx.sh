...
# ---------------------------------------------------------------------------
# Auto-include nginx-rendered.conf (= JA4 maps) inside http {}.
# /etc/nginx/conf.d/*.conf is read inside http {}, so dropping it here suffices.
# Use a "10-" prefix so it loads before the existing unmask-web-nginx
# package's unmask-web.conf (= alphabetical load order).
# ---------------------------------------------------------------------------
RENDERED_INC=/etc/nginx/conf.d/10-unmask-rendered.conf
cat > "$RENDERED_INC" <<'EOF'
# unmask-plugin-nginx auto: expand JA4 maps that use variables provided by the
# native module (= ngx_http_unmask_module) into http {}.  Generated by
# unmask-admin render-nginx.
# This file is dropped by the unmask-plugin-nginx package.  Overwritten on upgrade.
include /etc/unmask/nginx-rendered.conf;
EOF
chmod 0644 "$RENDERED_INC"
echo "  rendered include: $RENDERED_INC"
