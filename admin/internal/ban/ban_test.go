...
// TestAddWithSourceActionPersistsAction: an explicit per-row action passed to
// AddWithSourceAction is written verbatim into the ban file's action column, so
// native (ban-file "<key>|<source>|<action>" read) and forward-auth (banDecide
// rowAction) both enforce a honeypot per-preset override on EVERY subsequent
// request -- not just the first trip.  An empty action defers to the source
// resolver (= the live Honeypot.DefaultAction), identical to AddWithSource.
func TestAddWithSourceActionPersistsAction(t *testing.T) {
	dir := t.TempDir()
	d, err := db.Open(settings.DB{Driver: "sqlite", SQLitePath: filepath.Join(dir, "t.db")})
	if err != nil {
		t.Fatalf("db open: %v", err)
	...
	banFile := filepath.Join(dir, "ban.list")
	mgr := New(d, banFile, 0)
	// Stand in for the live honeypot DefaultAction that "" inherits at flush time.
	mgr.SetActionResolver(func(source string) string {
	...
	// explicit per-preset action -> written verbatim into the action column.
	mgr.AddWithSourceAction(ctx, "203.0.113.21", "", SourceHoneypot, "trap", "", "captcha_only")
	// empty action -> inherits the resolver default (same as AddWithSource).
	mgr.AddWithSourceAction(ctx, "203.0.113.22", "", SourceHoneypot, "trap", "", "")

	file := func() string { b, _ := os.ReadFile(banFile); return string(b) }()
	if !strings.Contains(file, "|honeypot|captcha_only") {
		t.Errorf("explicit per-row action not persisted to the action column\n---file---\n%s", file)
	}
	if !strings.Contains(file, "|honeypot|pow_then_captcha") {
		t.Errorf("empty action did not inherit the resolver default\n---file---\n%s", file)
	}
}
