...
func (h *Handler) settingsViewData(w http.ResponseWriter, r *http.Request, tab string) map[string]any {
	...
	return map[string]any{
		...
		"GatewayListenHTTP":  h.cfg().Gateway.ListenHTTP(),
		"GatewayListenHTTPS": h.cfg().Gateway.ListenHTTPS(),
		...
		// What an UNSET black-list picker falls back to.  The live refresh
		// needs it to answer "unset" without a round-trip, and the picker's
		// own label needs it to stop naming a value nothing resolves to.
		"UABlacklistFallback":        h.snapshotSettings().RateLimit.Default.ResolvedChallengeMode(),
