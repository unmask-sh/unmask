...
// `sudo cat <config-dir>/.setup-token` (= a sudoer on the host) can
// finish setup.  The file is deleted on completion so the wizard can't
// be re-run.
//
// It is a var, not a const: `unmask serve` overrides it (SetSetupTokenDir) to
// <dir-of-resolved-config>/.setup-token, so a relocated install
// (-config /opt/unmask/config.yml, or a second instance) reads its OWN token
// instead of the hard-coded /etc/unmask one.  The default still matches the
// package layout (config in /etc/unmask), so the postinstall keeps working.
var SetupTokenPath = "/etc/unmask/.setup-token"

const SetupTokenCookieName = "unmask_setup_token"

// SetSetupTokenDir points the setup-token file at dir/.setup-token.  Called by
// `unmask serve` with the directory of the resolved config file.  Empty dir is
// ignored (keeps the default), so a no-config dev run still has a sane path.
