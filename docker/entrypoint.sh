...
#
# Responsibilities:
#   1. If /etc/unmask/config.yml is missing, generate a minimal skeleton (= the
...
#     the rendered nginx config proxies /unmask/ to; compose sets unmask:9477
...
if [ ! -f "$CFG" ]; then
    ...
    cat > "$CFG" <<EOF
# unmask config (auto-generated by docker entrypoint).
...
${GATEWAY_REDIRECT_LINE}
    ...
    # container reaches this one by service name (compose: unmask:9477); a
