...
> subtle.  Measured on one production node over four days, 49,021 requests
> claimed to be ClaudeBot and 127 of them came from Anthropic's published
> ranges; the rest were scattered cloud addresses, many probing for
> `.aws/credentials` and `.env` under the borrowed name.  This release also
> adds `unmask stats`, a read-only report for the operator whose database is
> exactly the thing they cannot open.
