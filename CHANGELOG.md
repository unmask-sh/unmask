...
### Added
- (2026-08-19) **The notifications tab is visible — one tab, one save, four cards.**  The notification settings shipped complete — webhook form with test send, SMTP relay settings — with their nav entry commented out "until post-v0.1", which left the over-block breaker's alert channel unconfigurable from the UI: the breaker could trip, and its alert had nowhere to go.  One tab now carries it all, as separate cards under one form: the event selection (which gates webhook and mail equally), the webhook, the mail channel (its pause and recipients), and the SMTP server — a card of its own because the transport, which password-reset mail rides too, is not the same thing as the channel, but not a separate tab, because setting mail up should be one screen and one save.  The tab also states that the over-block alert has no per-event toggle (it is the fail-safe of last resort; mute it by disabling notifications as a whole, or the breaker itself in config.yml).
- (2026-08-19) **An admin account can be disabled without deleting it.**  A suspended account refuses sign-in (indistinguishably from a wrong password, and checked after the hash run — a distinct answer would confirm the account), loses any existing session on its very next request — faster and more certain than a password change — and drops out of the alert-mail recipients, while its role, email and history stay for re-enabling.  The last ENABLED superadmin cannot be suspended, demoted or deleted; a disabled superadmin no longer satisfies those guards, because an account that cannot sign in administers nothing.  Suspending your own account is refused (it would cut your session mid-click), and a disabled user's setup-link button is withheld.
- (2026-08-19) **New admin users can be invited by mail — without a password ever travelling by mail.**  The create form gains "email a setup link": the account is created under a random password nobody sees, and the invitee receives a one-time password-setup link (valid 72 hours, against the self-service hour — an invite is rarely opened the minute it is sent) through which they choose their own password.  Not even a temporary password exists to leak, which is why the link replaces the temporary-password pattern entirely.  The user list gains a per-row "send setup link" button for resending an invite or admin-side password recovery; sending it replaces any earlier link.  Both are offered only when SMTP is configured and the user has an email, and every send is audit-logged.
- (2026-08-19) **Alert mail can go to an explicit address list.**  `notifications.mail_to` (comma-separated, on the mail card) names the alert recipients directly; left empty, alerts keep going to the admin users' emails.  This separates the account contact (password reset, invites) from the alert destination — pointing alerts at a mailing list or a shared alerts@ box no longer means registering a pseudo-user to carry the address.  While mail_to is set, the per-user alert opt-out on the profile page is never consulted; it stays editable — the stored value matters again the moment mail_to is emptied — with an annotation saying why it is currently moot.
- (2026-08-19) **Each notification channel can be paused.**  The webhook and alert mail get their own pause toggle, so muting a channel no longer means blanking the webhook URL or the SMTP host and keeping the value somewhere to paste back later.  Pausing alert mail deliberately does not touch the SMTP transport itself — password-reset mail keeps working, so muting noisy alerts can never turn into a lockout.  doctor's over-block deliverability check counts a paused channel as configured-but-not-a-route.
- (2026-08-19) **`__hostname__` in notification labels.**  notifications.site_label and smtp.from_name expand the placeholder to the install's host id (server.host_id, falling back to the OS hostname) when the notifier and mailer are assembled — so a fleet ships one literal config fragment instead of baking each machine's name in, and an alert's label matches the events host column the operator would filter on.  The stored config keeps the placeholder; expansion happens at startup and on every save, and the test-send buttons expand it too, so a test shows exactly what a real alert would.
...
### Changed
- (2026-08-19) **The over-block breaker counts browser-grade traffic only.**  Serves with no User-Agent, or with a confirmed-bot JA4 verdict (`bot_*`), no longer feed the serves-per-IP ratio the breaker trips on: a scanner swarm is not a visitor trapped in a challenge loop, and its volume otherwise drowns the signal (a 2026-07-04 trip was 170 UA-less probes).  `suspect_*` verdicts stay in — browser-shaped traffic may be human.  The existing no-loads guard remains as the second layer.

### Fixed
- (2026-08-19) **`starttls: false` sent mail over STARTTLS anyway — and failed on the relay's certificate.**  The plaintext path delegated to stdlib `smtp.SendMail`, which upgrades opportunistically whenever the server merely advertises STARTTLS and then verifies the certificate — so plaintext to a localhost relay with a self-signed or expired certificate, the exact relay the setting exists for, failed on the certificate (observed on a fleet postfix).  The plaintext session now never upgrades; `starttls: true` keeps the opportunistic upgrade and the `insecure_skip_verify` escape hatch.
...
- (2026-08-18) **The retention page stops lying about which card is broken, and stops timing out.**  A timeout in the log-ingest card put its warning under the prune stats, which were sitting right there and correct; the flags are per card now.  And the row count that timed out is an O(1) estimate rather than an exact scan — a 25-day window's 1.6M rows outran the query budget on a modest host.
