...
### Fixed
- (2026-08-27) **The bot-hunt log's date cell painted over the IP next to it.**  The events table is a fixed layout, so the width declared on a column header is the column -- nothing in it can push it wider -- and the date cell is set to never wrap and never clip.  Those two together have no visible failure mode until something does not fit, at which point the overflow is simply drawn on top of the neighbouring cell.  The cell held a timestamp and a site badge, which already reached a few pixels past the edge, and a third element on rows recorded under more than one host id: a pill naming the node.  A node writes one host id forever, so that pill is normally invisible and appears only after a rename -- which is exactly when an operator is least expecting the IP column to go missing.  The node's host id has moved into the popover the timestamp already opens, as its own labelled row, where it costs no width and sits beside the Ref ID, the vhost, the port and the referer.  The column is wider by the space that frees up, and the site badge is allowed to use more of it than the cap the same badge needs in the live stream.  The cell also clips now, which is the part that actually holds: how much width a timestamp needs depends on which monospace font the viewer's machine resolves, and that is not knowable when the width is written -- the same timestamp measured 121.6px under one of them and 146.4px under another, on the same page.  A column sized against one font is not sized against the other, so the width decides whether anything is cut, and the clipping decides that being cut is all that happens.  Nothing became unreachable: which hosts a database holds is what the host picker in the top bar has always listed.  The popover row that showed the visitor's Host header is now labelled Site, because the two things this page calls host and site are the two things its filters call host and site, and one of them was answering to the other's name a few pixels from a pill that meant the opposite.

- (2026-08-25) **The challenge page could send a visitor off-site.**  Completing a challenge at `/unmask/challenge/` returns the visitor to wherever `?_test_redirect=` says, and what kept that on your own site was three checks on the string: it starts with `/`, its second character is not `/`, and its second character is not `\`.  That reads as exhaustive and is not, because it inspects the string as written while the browser inspects the string as parsed, and those differ -- a URL parser removes TAB, CR and LF from anywhere in the input before it parses anything.  `/<TAB>/evil.example` passes all three checks and is then navigated as `//evil.example`: protocol-relative, off your origin, on any install.  The name of the parameter suggests otherwise, but nothing gated it behind a test flag -- the branch it lives in is chosen by the path being the challenge page, which is the page every challenged visitor is served.  The value is now resolved against the page's own origin and the two origins compared, which is the only form that agrees with the navigation that follows: asking the parser what a string means, rather than predicting it.  The same guard in the same shape sat in front of five redirects on the Go side -- the observe-only bounce, the rebind bounce, the challenge return and the admin login return among them -- and was replaced the same way, with control characters refused outright rather than reasoned about.  A browser-level test drives the real challenge through six values that resolve off-site, plus a legitimate one, because this was invisible to every request-level test and stayed that way for the length of its life.  In the same pass, request-supplied values reaching the daemon's own log -- addresses, user agents, paths -- are stripped of line breaks and control characters before they are written, so a crafted request can no longer forge a log line that appears to come from unmask.

- (2026-08-25) **Alert mail could not have a header written into it.**  Every header line the mailer emits ends at a CRLF, and the values going into them were passed through unchanged when they were plain ASCII — which CR and LF are.  A value carrying one therefore stopped being a value: what followed was read as the next header, and after a blank line, as the body.  `net/smtp` refuses CR and LF in the SMTP commands, so `MAIL FROM` and `RCPT TO` were never at risk, but the message itself travels in the DATA payload, which nothing inspected.  The reachable source is a notification subject, which names the address or the site an event came from — so whoever caused the event chose part of a mail header.  Every header value is now filtered where it is written rather than at each caller: CR and LF become spaces, so a value that had a line break still reads as separate words, and the remaining control characters are dropped.  The body needs none of this — it sits after the blank line, and the SMTP writer already dot-stuffs it.

- (2026-08-23) **doctor stopped telling operators the opposite of what happens to a customised challenge asset.**  A copy of `challenge.html` / `challenge.js` under `/usr/share/unmask/challenge/` used to win at serve time, and doctor said so: "the deployed file WINS at serve time, so visitors are running that version, not this one".  0.1.32 reversed that — the embedded assets became authoritative and the packaged copy is read by nothing — and this message kept its old wording, so for three weeks it told operators their visitors were on a file the runtime had stopped opening.  A check that is confidently wrong costs more than one that is silent: it sends someone redeploying package assets to fix a problem they do not have.  It now says what is true, that the copy on disk reaches nobody, and names the two ways out (delete it, or point `challenge_html_path` / `challenge_js_path` at it if the customisation was deliberate).  It is also no longer a warning.  doctor has two levels and a warning has to mean "this needs attention"; nothing here does, because the install serves this binary's assets whatever sits in that directory — and since the packaging drops that file as a template to copy, a binary upgraded ahead of its package makes it differ with no mistake involved.  On an install upgraded by replacing the binary that was every run, forever, which is how a tool teaches people to stop reading it.
...
### Changed
- (2026-08-21) **Every unset axis action now resolves to the same chain.**  header-integrity, stale-browser, manual bans and the community feed each resolved an unset action to `captcha_only`; they now resolve to `pow_then_captcha`, which is what the honeypot tier and the rate-limit default already did.  Both end in a CAPTCHA, so the same clients are stopped and the same pass grade is demanded — the difference is what the operator can see afterwards.  Straight to the CAPTCHA records one outcome, solved or not; running the proof-of-work first splits it in three: a client that never reaches `pow_pass` could not execute the JavaScript at all, one that clears the work and abandons the CAPTCHA is the headless PoW-solver these tiers exist for, and one that clears both is a person.  That distinction is the whole question at these two tiers — a Chromium user-agent with no client hints is either a spoof or a proxy that stripped the header; a frozen major is either a pinned scraper or somebody on an old browser.  The cost is a transparent PoW leg for a misclassified person who was going to be shown a CAPTCHA anyway, and that leg mints no cookie of its own.  For the two ban sources the reason is consistency rather than diagnosis: an operator reading a config should not have to remember which tier is the exception, and a manual ban placed from the bot-hunt log with one click is the entry most likely to be wrong — it still clears with the same CAPTCHA, one transparent leg later.  An explicit `captcha_only` is unchanged and still wins everywhere.  Installs that never touched these actions pick the new behaviour up on upgrade — which is what leaving them unset means.
- (2026-08-21) **The statistics tables' user-agent columns read like the hunt log's.**  They held the raw string — several hundred pixels of `Mozilla/5.0 (…) AppleWebKit/537.36` that filled the column, distinguished nothing between rows, and clipped exactly where the useful part lives.  The hunt table settled this long ago: a platform glyph and a short reading (`Windows 10 · Chrome 126`), a badge for crawlers, the full agent one click away.  That cell is now a shared definition, and every user-agent column on the statistics page renders through it — the cookie-reuse rankings, the CAPTCHA pass and fail tables, the stealth card.  The popover follows: hovering shows the same short reading as its heading and pinning puts it in the title bar, where the raw agent used to sit and was no title at all.
- (2026-08-21) **One token for the absence of a verdict — `(none)`.**  The same unlabelled request wore four costumes across the admin: `ok` in the hunt log, `none` in the funnel, `(none)` in the verdict distribution, `-` in the pass tables.  `ok` was the worst of them — events store NULL when no rule matched, so the cell asserted an evaluation nobody performed, in a word a rule can genuinely be named — and the cookie-reuse rankings, which resolve each stored fingerprint against the CURRENT rules at render time, printed it for rows whose rules had simply moved underneath them.  Every verdict column now renders the absence as `(none)`, muted where the table is dense.  The funnel's permanently-empty `ok` row retires with it: current versions record a miss as NULL, so that seat was reserved for a guest that stopped coming, and where older data still holds the literal `ok` it rides as an ordinary observed verdict with no special place.  `(none)` and `ok` both sort last, after every named verdict, so the absence of a signal never reads as a peer of the signals.  The help texts follow the data rather than the original guess — `(none)` does not mean "no fingerprint captured", it is every request that earned no verdict name, capture failures included.  The reuse table's column header also stops translating: `verdict` names an axis the settings tab, the funnel and the hunt log all speak, so rendering it as 判定 in one place made it read like a different concept.

### Fixed
- (2026-08-21) **A rebind lineage could be forgotten while its CAPTCHA cookie was still valid.**  The 8-day cap on the cookie-reuse table and the 8-day idle cutoff on rebind lineages were both reasoned from the 3-day cookie of the codebase this grew out of; unmask's own defaults have always been 7 days for proof-of-work and 14 for a CAPTCHA.  A lineage could therefore be pruned while its cookie still lived, and the next rebind opened a fresh row — resetting the per-lineage limits the cutoff exists to enforce.  Both windows go to 15 days, past both cookie defaults, which also leaves the reuse card two full proof-of-work lifetimes to describe instead of barely one.  The card's description follows, at the cost of a table roughly twice the size.

## [0.1.34] - 2026-08-21

### Fixed (security)
- (2026-08-20) **A proof-of-work cookie no longer satisfies a bot-verdict JA4 fingerprint.**  The JA4 verdict axis was the one CAPTCHA-grade source missing from the pass-cookie gate on both wires: a fingerprint the operator marked `bot` was CAPTCHA-gated only while the client arrived bare, and any proof-of-work cookie — minted under another fingerprint, or through a rotating proxy exit before the rule existed — sailed past the rule for the cookie's remaining lifetime.  Observed live: a residential-proxy herd rode proxy-solved proof-of-work cookies through content pages in bulk while never once clearing the CAPTCHA its fingerprint rule demanded.  The fingerprint now joins the UA / protected-path / community-feed / geo / ASN sources in `$unmask_needs_captcha_grade` (native) and the forward-auth veto, so such a cookie is refused and the fingerprint's own chain is served instead.  Native installs need `render-nginx` + reload for the new map.

### Changed
- (2026-08-20) **The stats range picker goes below a day: 1h / 3h / 6h / 12h.**  Judging a rule you changed ten minutes ago against a 24h window means reading mostly pre-change traffic — the funnel says what the day looked like, not what the change did.  The four sub-day presets sit alongside 24h and are offered unconditionally (every install has the last hour).  Sub-day windows deliberately bypass the hourly rollup: its finest grain is an hour and its window clause is inclusive at both ends, so a "1h" read served from it would carry up to an hour of pre-change traffic on either side — exactly what the operator is trying to exclude.  They fall through to the raw-event scan instead, which bounds on exact timestamps and, over half a day at most, costs less than the filtered-view scans the page already runs.  The AI-crawler breakdown keeps reading the rollup (it has no raw path by design, and a crawler roster is not what a just-changed rule is judged by).  Every trailing preset now resolves through one token table (`RangeHours`), so the picker, the queries and the window cannot disagree about what a token spans.
- (2026-08-20) **A bot-verdict fingerprint with no configured chain now inherits the operating default — on every consumer, from one resolver.**  The axis's effective chain is resolved in ONE place for the check, the serve, the pass-cookie gate and the render (`EffectiveJA4BotChain`: the JA4 tab's default → preset override → row action, else the operating default chain — `pow_then_captcha` on a fresh install), which is what the settings UI always claimed the unset option meant.  Before, the two wires disagreed about that fallback: forward-auth hardcoded `captcha_only` while native served the operating default — and once the grade gate exists, a gate/serve disagreement is a challenge loop that re-mints the very cookie the gate refuses.  Inheriting rather than pinning keeps one knob in charge, and the default's proof-of-work leg is deliberate: it costs a person nothing, costs a bot fleet real compute, and its funnel separates the bots that cannot even run JavaScript (PoW stays zero) from the ones that can — one more axis of evidence per fingerprint.  An explicit `pow_only` row (or a `pow_only` operating default) remains the opt-out; forward-auth installs that relied on the old implicit `captcha_only` can pin it in the JA4 tab's default.

### Fixed
- (2026-08-20) **A pass rate stops rounding up to a figure its own row contradicts.**  The AI/crawler card read `100.0%` on a row whose Challenged column was not zero — any rate above 99.95% rounds up under `%.1f`, so a row could claim a perfect pass rate while the column beside it said otherwise.  A table that argues with itself in one row costs the reader more than the decimal saves — they trust the smaller number and distrust the rest of the card.  `100.0%` and `0.0%` are now reserved for the exact values; anything short of them reads `>99.9%` or `<0.1%`, the same shape the composition bar already used at the floor.
- (2026-08-20) **A bot-hunt ranking can no longer stretch the row it sits in.**  The ranking cards share one row but their cardinalities are nothing alike — a busy window holds thirty distinct addresses against a handful of TLS fingerprints — so the address card alone set the height, left two thirds of the space beside it blank, and pushed the event log that far down the page.  A shared cap scrolls the overflow inside the card instead, with the header staying put so a scrolled column still says what it counts.
- (2026-08-20) **Four app user-agent labels read in English.**  `Google アプリ`, `Yahoo! JAPAN アプリ`, `アプリ内ブラウザ` and `アプリ (Dalvik)` were Japanese string literals in the classifier, which has no language to switch on, so they rendered as Japanese in an English admin — one row an operator could not identify beside rows they could.  Japanese belongs in i18n, where the rest of the admin's Japanese lives.

## [0.1.33] - 2026-08-20

- (2026-08-20) **A headless browser could clear the behavioural CAPTCHA at full marks.**  The check read the mouse trail's geometry and ignored its clock, though every point already carried a timestamp — so a driver that shaped a curve scored a clean 1.0, and no threshold could be moved to catch it because it was already at the top.  What it could not fake was how the movement was built: a driver interpolating a path divides each leg by a step count and emits that one delta for the whole leg, while an arm accelerating and settling between clock-driven samples produces a different length almost every time.  Checked against a real automation-driven Chrome rather than a fixture — its timing jittered like any other input, so the tell was 69 segments carrying 8 distinct lengths.  That browser now scores 0.4 where it scored 1.0.  Two cheaper tells ride along: input the page dispatched to itself (a browser sets `isTrusted` and a page cannot forge it upward), and a phone that reports no touch capability while a cursor crosses it.
- (2026-08-20) **The numeric-add fallback stopped handing out CAPTCHA grade to scripts.**  It exists for visitors the behavioural check could not confirm, and it asked for arithmetic and nothing else — the one task a headless browser does better than the person the fallback is for.  What separates them is not the answer but how it reached the field: a person entering a number produces input events on every input method there is, a script assigning `.value` produces none.  A pass with no input events behind it is still a pass, but it is issued at proof-of-work grade instead of CAPTCHA grade — it clears the ordinary posture and fails exactly where an operator demanded a CAPTCHA.  A challenge page cached from before this ships keeps the full grade, so an upgrade strands nobody mid-answer.
- (2026-08-20) **A refusal says which axis refused.**  Every deny an axis made logged as `axis_deny`: true, and useless on the page whose job is telling the operator which rule did it.  Denials now read `geo_deny` / `asn_deny` / `ua_deny` / `ja4_deny` / `community_deny` and carry the entry that matched — an operator running a dozen ASN rules needs the one that fired.  The hunt filter gains those values; until now the reason dropdown could not select refusals at all.
...
- (2026-08-18) **The hub feeds are signed.**  Bypass ranges are trust data — an address inside them walks past the challenge — so their integrity no longer rests on the transport alone: each document is published with a detached ed25519 signature produced on a host that is not the one serving it, and the daemon refuses a document whose signature does not match.  That is what makes an escape hatch offerable for hosts whose trust store cannot verify the hub: `sync_insecure_tls` waives the transport check and makes the signature mandatory in exchange.  Ingest also refuses a document older than the one already held, so a replayed old-but-signed document cannot reopen ranges a vendor has rotated away from.
- (2026-08-18) **Range updates without a network.**  `update-iprange -file` imports the aggregated document from a local copy, for hosts that cannot make the pull at all; `unmask sign-feed` signs and verifies one, for anyone running their own hub.  A running daemon now notices override files another process wrote, which closes a quiet failure: a command-line sync followed by a settings save used to re-render nginx from the daemon's startup snapshot and roll the update back.
- (2026-08-18) **Bypass presets follow the user-agent policy you already set.**  An install that saved its preset list once keeps exactly that list, so a range preset added in a later release sat unchecked forever and its vendor stayed on name-only rescue — observed twice in one week, once challenging a genuine crawler for a month, once passing every impostor.  A vendor whose crawlers already pass by name now gets its published ranges enabled automatically, which flips it to address verification: the only visitors whose fate changes are the impostors.  Unanimity is required (every pattern the preset backs must currently pass), the address data must be fresh, and the derivation is never written into your config — opting a preset out is its own recorded decision.
- (2026-08-18) **The retention page stops lying about which card is broken, and stops timing out.**  A timeout in the log-ingest card put its warning under the prune stats, which were sitting right there and correct; the flags are per card now.  And the row count that timed out is an O(1) estimate rather than an exact scan — a 25-day window's millions of rows outran the query budget on a modest host.
- (2026-08-18) **The release refuses to ship a stale range snapshot.**  Refreshing the ranges compiled into the binary is a step a person has to run, and it was skipped for six weeks — nothing failed, the binary just quietly shipped June's ranges into August, and installs that cannot sync served refusals to genuine crawlers.  A snapshot older than a week now fails the release instead.

## [0.1.32] - 2026-08-18

- (2026-08-16) **A CAPTCHA pass is a pass, not proof of a person.**  The overview's CAPTCHA tile counted every request that presented a CAPTCHA-grade cookie and captioned the sum as humans, which conflated two things an operator needs apart: how many CAPTCHAs were actually solved, and how much traffic then rode the cookies those solves minted.  The tile now separates solves from returning cookie traffic, and both the CAPTCHA and proof-of-work tiles carry a `?` popover saying exactly what is counted — the caveat that used to sit inline as a parenthesis nobody could act on.  The CAPTCHA report's `ok` column is likewise named for what it is: a TLS-fingerprint grade, not a claim about the species of the visitor.
- (2026-08-16) **Truncation is measured in a way every browser agrees with.**  Whether a clamped cell needs a popover was read from `scrollWidth`, and Gecko answers that question differently for table cells — the overflow never registered, so on Firefox the underline and the popover with the full value simply were not there.  The measurement is a DOM Range now, which reports the laid-out width of the content itself in every engine.  While in there: everything that hides a popover shows the same dotted underline (an affordance that existed on some cells and not others), a hover popover can be reached by the pointer without closing on the way, the marker re-checks after fonts and late layout instead of trusting the first paint, and the shared popover assets are cache-busted so none of these fixes wait behind a browser's disk cache.
- (2026-08-16) **Admin tables come in two sizes, on purpose.**  Four slightly different font sizes had accumulated across the hunt, bans and statistics tables, one per page that ever tweaked one.  There are now two deliberate tiers, and a request line hundreds of characters long — a ranking's path, a ban reason that is really an injection probe — is clipped inside its cell with the full value one hover away, instead of setting the width of the whole table.
...
- (2026-08-15) **The timeline says which clock each line came from.**  Session times reconstructed from the browser's own interval measurements (0.1.31's fix for beacons racing over the wire) are marked with `≈` centred on the line they belong to, with a legend explaining that these times came from the page's clock and will not match an access log to the millisecond — silence about that read as the log being wrong.  Server-stamped lines carry no mark, so the mark means something.
- (2026-08-15) **An abandon pill says whether the visitor ever got in.**  The mark introduced in 0.1.31 answered "did anything arrive afterwards", which conflated a Back-button retreat with a solve that landed elsewhere.  It now answers the question the operator is actually asking of an abandon: did this client end up passing (`✓` — a later beacon shows a pass) or not (`∅`), and it sits at the front of the pill where it stays visible when the pill overflows its column.
- (2026-08-15) **A ranking that could not be read says so.**  The hunt page's rankings share one statement timeout, and a query that exceeded it rendered as an empty section — indistinguishable from "nothing matched", which on a page used mid-incident is a dangerous thing to fake.  Each ranking that failed now says it was unavailable, the ones that succeeded render normally, and the lone-row session popover shows the time instead of repeating the hostname beside it.
- (2026-08-15) **A geo or ASN rule's chain is the chain that gets served.**  0.1.31 taught the pass-cookie gate about the network axes; this closes the serving half.  On a native install the challenge endpoint attributed the serve to the geo/ASN rule that fired and then handed out the site's default chain anyway — a rule saying `captcha_only` was served as `pow_then_captcha`, and the proof-of-work leg is precisely what an operator picking `captcha_only` was declining to offer that network.  The serve now uses the same resolution that named the axis, for exact-ASN rules, organisation rules, provider presets and country rules alike.
- (2026-08-15) **The built-in challenge assets are what runs, unless someone says otherwise.**  A copy of the challenge page dropped under `/usr/share/unmask/challenge/` silently outranked the assets embedded in the binary, so an upgrade could ship a new page and an old overlay kept serving it — invisibly, because nothing said an override was in effect.  The embedded assets win now; the only way to serve a custom page is the explicit `challenge_html_path` / `challenge_js_path` setting, and a stray overlay file that would previously have taken over is warned about, once, at startup.

## [0.1.31] - 2026-08-15

- (2026-08-15) **A session that ran exactly as served was reported as having diverged.**  The bot-hunt log's session popover leads with the chain that was offered and, when the visitor met something else, what they actually got.  It decided that from the timeline — any CAPTCHA on a chain that was not `captcha_only` counted — so `pow_then_captcha`, a chain whose definition is "proof of work, then a CAPTCHA", was announced as a departure the moment it reached its second leg.  Every CAPTCHA beacon already records *why* it appeared, including the one case the server cannot know at serve time (a client that trips the bot-flag threshold after a retry and is walked into a CAPTCHA the chain never offered).  That is now what the line reads, so only a real escalation is called out.
- (2026-08-15) **What a visitor did after abandoning now rides inside the abandon pill.**  Whether anything arrived from that address in the next 30 seconds — the only way to tell Back from a closed tab — was a separate chip beside the pill: it wrapped onto its own line on a narrow column and read as another step in the chain, and in Japanese its label was simply the word "abandon" again.  It is now one character at the front of the pill: `↩` when a request followed, `∅` when none did.
- (2026-08-14) **A honeypot ban says which host was probed, not just which path.**  On an install serving several vhosts, `/cgi-bin/…` exists on every site that has one, so the path alone could not answer which site was being scanned.  Both deployment modes already had the host in hand and dropped it.  The reason is now built by one function for both, with the host budgeted so a scanner sending a kilobyte of path cannot push it out of the stored column.
- (2026-08-14) **A ban reason that is really a request line no longer sets the width of the whole table.**  Honeypot bans record the path that tripped them, and the paths that trip them are injection probes hundreds of percent-encoded characters long; in an auto-layout table one such row decided the reason column's width for every row.  The reason is clamped with the full value one hover away, and the columns that hold an address, a fingerprint or an action name — none of which have a sensible break point — no longer wrap, so a row is one line again.
- (2026-08-14) **The bot-hunt log orders a session by when the browser sent each beacon, not when the server received it.**  Phases that leave in the same JavaScript tick — a proof-of-work pass and the CAPTCHA it hands off to — travel as separate requests and are stamped on arrival, so which lands first is a race the network decides, and it does invert: a session could show a CAPTCHA recorded before the proof of work that led to it.  Beacons now carry the sequence number the page counted, and the popover's timeline is spaced by the intervals the page measured rather than the intervals the wire produced.  The absolute clock stays the server's, and nothing a page reports can move a row on the timeline every filter and rollup reads.
- (2026-08-14) **A geo or ASN rule set to a CAPTCHA was satisfied by a proof-of-work cookie.**  The pass-cookie exemption asks whether a request must be backed by a CAPTCHA-grade cookie, and consulted three sources — the user-agent chain, protected paths and the community feed.  The two network axes were never wired in, on either deployment mode, so an address already holding a proof-of-work cookie minted under a global `pow_only` posture walked past a rule that said CAPTCHA for the cookie's remaining lifetime.  Both modes now fold the geo and ASN axes into the same grade requirement, minus each axis's own exempt paths, and rules in rate-limit mode are unaffected.
- (2026-08-14) **An ASN rule matching on organisation rendered a config nginx refuses to load.**  An organisation pattern can contain a space, and the geo block emitted the value unquoted, so nginx read the space as a third parameter and rejected the entire rendered file.  The failure order was the worst available: the render succeeded and `nginx -t` failed afterwards, leaving an install one restart away from not coming back up.  Values are quoted now; exact-ASN rules were never affected.
- (2026-08-14) **The built-in CAPTCHA is the recommended one.**  An external provider has to be told which domains may use it, and an admin panel reached on a host that was never registered there fails the check — locking the operator out of the settings page they would use to change the setting.  The built-in behavioural CAPTCHA has no such registration, and its pass rate in practice does not argue for the trade.
- (2026-08-13) **The statistics page offers only the spans it can fill.**  A range longer than the data on hand rendered as an empty stretch that looked like an outage rather than a page that had not been running that long.  The choices now stop at the oldest hour actually stored, taking both the raw events and the hourly rollups into account.
...
### Added
- (2026-08-12) **Anthropic's crawlers are verified by their published IP ranges.**  One new preset (`claude`) carries the official list -- a single shared document for ClaudeBot, Claude-User and Claude-SearchBot -- and the five Anthropic user-agent patterns ride it: with the preset on, the name alone no longer passes, only an address inside the ranges does, exactly as Googlebot / GPTBot / Applebot already work.  The retired Claude-Web and anthropic-ai names are covered on purpose: genuine traffic for them no longer exists, so range verification simply turns their spoofs into challenges (the Google Web Preview precedent).  Fresh installs get the preset on by default; existing installs keep their saved preset list, and while the preset is off the user-agent rescue stays -- nothing tightens on upgrade until you opt in.
- (2026-08-12) **`unmask stats` -- the numbers, without opening the database.**  A read-only aggregate dump for hosts where a database client is unavailable or unwise: CentOS 6's sqlite3 predates WAL and refuses the file outright, a MariaDB's credentials may live only in config.yml.  Six reports over one window -- request composition, events by phase and by JA4 verdict, top addresses / user-agents / TLS fingerprints -- with `-site` scoping and a `-tsv` mode that emits one flat record per line for `awk`, keeping commentary on stderr where a parser cannot mistake it for data.  It only reads; nothing it does can change the database.

### Fixed
- (2026-08-12) **The over-block breaker no longer trips on a scanner farm.**  Its signal was challenges-served per distinct address, and a swarm of web-shell probes from a handful of cloud addresses produces exactly the ratio it watched for, while never loading a single challenge page.  A visitor stuck in a loop loads every challenge they are served; a scanner loads none.  The breaker now requires the loads to corroborate the ratio before it declares over-blocking, so it cannot advise an operator to loosen enforcement against a scan that the product is correctly refusing.  The banner and the breaker read the same figures, so they can no longer diverge.
- (2026-08-12) **The traffic-composition card stopped hiding its breakdown behind a wrong diagnosis.**  When the sum of the named buckets exceeded the total, the card replaced the whole breakdown with a double-counting warning.  The arithmetic actually closes -- the counters are minute-bucketed while two terms are hourly, so a small skew near bucket edges is expected -- and the card now shows the breakdown with the residual attributed honestly, reserving the warning for the one case that is real (the counter feed being off entirely, which is also the only case the served-challenges fallback is for).
- (2026-08-12) **A hunt row names its Host when the reader cannot infer it.**  With sites declared, a row whose Host is outside every declared site showed an address with no context -- it looked like a filter leak.  Ghost hosts now carry an explicit badge (amber, distinct from the multi-site badge), and collapsing a session no longer throws away the phase cell's badges: the rate-limit zone, the LB-misconfiguration warning, the returned/gone marker and the reload counter all survive the collapse.
- (2026-08-12) **`unmask doctor` no longer tells you to run it with sudo.**  The advice was unreachable: doctor drops privileges before the check that would have printed it, so the euid it tested was never 0.  The permission notes now name the actual remedy (group membership for the paths involved) instead of a command that recreates root-owned state the next release would trip over.
...
### Fixed
- (2026-08-10) **A protected path whose mode ends in a CAPTCHA was satisfied by a proof-of-work cookie.**  The `unmask itself` preset ships on by default and puts a challenge in front of `/unmask/admin/`, so this is the gate on the product's own login page.  The pass-cookie exemption asked for a CAPTCHA-grade cookie only when the *user-agent* sat on a captcha-ending rule; it never consulted the protected path.  A client holding a proof-of-work cookie -- which a headless solver mints for itself and then rides freely through a rule that said CAPTCHA -- therefore walked through a captcha-graded protected path under an ordinary browser user-agent.  Both deployment modes now fold the protected axis into the same grade requirement, so `captcha` and `pow_then_captcha` paths accept only a genuine CAPTCHA pass.  A `pow` path is unchanged, and nobody who actually cleared the gate is affected: clearing a captcha-ending chain issues a CAPTCHA-grade cookie.
...
- (2026-08-06) **The abandonment rate counted the crawlers the operator had aimed a rule at, so on a site with bot traffic it read as a catastrophe.**  Anything a rule sends to the challenge is expected to leave without solving -- that is the rule working -- but those departures were divided into the same rate offered as "how many people give up on the challenge".  On a site with heavy bot traffic the rate read as near-total abandonment: almost every challenge load in the window was a rule-targeted crawler, and the ordinary visitors underneath them were invisible.  The rate is now computed over the visitors who arrived at a challenge without a rule pointing at them, which is the only population the number is ever used to reason about, and it reads 4.8% on the same window.  The rule-targeted departures are still counted, under the blocked figure where they belong.
...
### Fixed
- (2026-08-05) **"deny" did not deny.**  Every axis except the ban was consulted only when the visitor held no pass cookie, so the word meant "deny unless this client cleared a challenge at some point in the last week" -- and against anything able to clear one, that is not a block at all.  Seen on a production install: a crawler the operator had already removed from the rescue list solved the proof-of-work across its whole address pool and then served itself freely for a day through a UA row set to deny.  Raising the difficulty cannot reach it either, because the cookie lasts a week and the cost is one solve per address per week whatever the difficulty is.  A resolved deny is now enforced beside the ban, ahead of anything that reads the cookie, on both the native and forward-auth paths.  Rescues still win: a listed crawler, a bypass IP and a bypass path are all checked first, so nothing an operator exempted on purpose is affected.
- (2026-08-05) **The overview counted some requests twice, and could not say how much of the traffic was human.**  A request carrying a pass cookie that also matched a bypass rule counted as both, which on an install serving its own assets from bypassed paths is a large share of the day, and the excess tracked each site's bypass-path share exactly.  The classification is now one decision rather than three that happened not to overlap, and the human share is a count of requests that arrived holding a pass cookie rather than whatever was left over.  What is genuinely unattributed gets its own segment with a breakdown, so the abandons stop being quietly counted as people who got through.  Verified on a fleet node before and after: the parts used to overshoot the total and now sum to exactly it.
...
### Fixed
- (2026-08-02) **A visitor that sends no User-Agent was recorded — and classified — as unmask's own fetcher.**  The forward-auth protocol reports an absent UA as a header the proxy set to the empty string, and the resolver read empty as absent and fell back to the subrequest's own User-Agent, which under Apache is whatever LuaSocket sends.  On the install where this surfaced a large share of all events carried that name, and the sites in question are hit mostly by scanners that send no UA at all — exactly the population the check exists to catch.  nginx's `auth_request` inherits the client's headers, so its subrequest carried the visitor's UA and the substitution was invisible there.
...
### Fixed
- (2026-06-19) **`ja4_mismatch` false-positives from HTTP/2 ↔ HTTP/3 drift on roaming rebind.**  A real browser presents a different JA4 over HTTP/2 (TCP) than over HTTP/3 (QUIC) — they differ in every field — and the single-JA4 `_bvj` refused a silent rebind on whichever transport it had not been minted under, so on a production node -- where the rejects were daily and none of them were bots -- a large share of authenticated roaming rebinds fell through to a needless challenge.  `_bvj` now stores a `~`-joined set of JA4s — one appended per transport, capped at 3 — and matches any member, and a non-bot verdict soft-passes.  Go-only; the native C plugin never parses `_bvj`.
