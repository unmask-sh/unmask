...
### Changed
- (2026-09-16) **The collateral of a fingerprint ban is answered from an index.**  Asking how many addresses completed the challenge with a given JA4 in the last seven days used to walk the whole retention window, which with the default retention is the whole event table.  A new index on the fingerprint and the date turns each question into one range seek.
